-- Praise posts get a "Hallelujah" reaction open to everyone (not just
-- leader roles, unlike praying for a request) — celebrating a testimony
-- is a whole-church thing, not a pastoral duty. Request posts keep the
-- existing leader-only "I prayed" behavior unchanged.
...
begin
  ...
  if req.kind = 'request' and me_role not in ('pastor','agricultural_minister','worship_minister','lead_pastor','owner') then
...
create policy "prayer_log_insert" on public.prayer_log
  for insert with check (
    prayed_by = auth.uid()
    and (
      exists (select 1 from public.requests r where r.id = request_id and r.kind = 'praise')
      or exists (
